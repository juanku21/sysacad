...
export class FinalExamRegistrationController {
    ...
    public static get : RequestHandler = async (req : Request, res : Response) => {
        try {
            const result = await FinalExamRegistrationService.get()
    ...
    public static getById : RequestHandler = async (req : Request, res : Response) => {
        ...
        try {
            const result = await FinalExamRegistrationService.getById(id)
    ...
    public static create : RequestHandler = async (req : Request, res : Response) => {
        ...
        if (!FinalExamRegistrationValidator.create(city)) {
            res.status(400).json({error: 'Los datos enviados son incorrectos'})
        }

        try {
            const result = await FinalExamRegistrationService.create(city)
    ...
    public static update : RequestHandler = async (req : Request, res : Response) => {
        ...
        if (!FinalExamRegistrationValidator.update(city)) {
            res.status(400).json({error: 'Los datos enviados son incorrectos'})
        }

        try {
            const result = await FinalExamRegistrationService.update(id, city)
    ...
    public static delete : RequestHandler = async (req : Request, res : Response) => {
        ...
        try {
            ...
            const result = await FinalExamRegistrationService.delete(id)
