...
export class CourseRegistrationController {
    ...
    public static getById : RequestHandler = async (req : Request, res : Response) => {
        ...
        try {
            const result = await CourseRegistrationService.getById(id)
    ...
    public static create : RequestHandler = async (req : Request, res : Response) => {
        ...
        const courseRegistration = req.body

        if (!CourseRegistrationValidator.create(courseRegistration)) {
        ...
        try {
            ...
            const result = await CourseRegistrationService.create(courseRegistration)
    ...
    public static update : RequestHandler = async (req : Request, res : Response) => {
        ...
        if (!CourseRegistrationValidator.update(courseRegistration)) {
            res.status(400).json({error: 'Los datos enviados son incorrectos'})
        }
        ...
        try {
            ...
            const result = await CourseRegistrationService.update(id, courseRegistration)
    ...
    public static delete : RequestHandler = async (req : Request, res : Response) => {
        ...
        try {
            ...
            const result = await CourseRegistrationService.delete(id)
