...
import { EducationalOfferValidator } from "../validators/educationalOffer.validator"
import { IdEncrypter } from "../utils/encryption"
import { HeaderStrategy } from "../utils/headerStrategy"


export class EducationalOfferController {

    public static get : RequestHandler = async (req : Request, res : Response) => {
        try {
        
            const strategy = new HeaderStrategy(EducationalOfferService)
    ...
    public static getById : RequestHandler = async (req : Request, res : Response) => {
        ...
        try {
            const result = await EducationalOfferService.getById(id)
    ...
    public static create : RequestHandler = async (req : Request, res : Response) => {
        ...
        if (!EducationalOfferValidator.create(city)) {
            res.status(400).json({error: 'Los datos enviados son incorrectos'})
        }

        try {
            const result = await EducationalOfferService.create(city)
    ...
    public static update : RequestHandler = async (req : Request, res : Response) => {
        ...
        if (!EducationalOfferValidator.update(city)) {
            res.status(400).json({error: 'Los datos enviados son incorrectos'})
        }

        try {
            const result = await EducationalOfferService.update(id, city)
    ...
    public static delete : RequestHandler = async (req : Request, res : Response) => {
        ...
        try {
            ...
            const result = await EducationalOfferService.delete(id)
